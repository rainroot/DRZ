...
{	
	...
	struct timespec Tsleeptime;
	Tsleeptime.tv_sec = 0;
	Tsleeptime.tv_nsec = 1000000;
	...
	while(loop){
		...
		if(get_pitd != NULL){
			...
			if(get_pitd->NPT_send_ok == SEND_OK){
				if(get_pitd->packet_type == TUN_PACKET){
					if(get_pitd->NPT_packet_status == PACKET_DROP || get_pitd->TPT_packet_status == PACKET_DROP){
						...
						pthread_mutex_lock(&pipe_epd->nit->net_idx_mutex);
						net_idx = get_pitd->net_epd->nit->net_idx;
						pthread_mutex_unlock(&pipe_epd->nit->net_idx_mutex);
#if 0
						if((net_idx + 1) == get_pitd->data_send_idx){
							...
							printf("#%s %d # net_idx %lld data_send_idx %lld key %lld \n",
									__func__,__LINE__,
									net_idx,
									get_pitd->data_send_idx,
									tmp_pitd.key);

							data_send_idx_bool = false;
						}
#else
							if(pipe_epd != NULL && pipe_epd->n_fdd != NULL){
								pthread_mutex_lock(&pipe_epd->n_fdd->net_w_mutex);
			...
			if(data_send_idx_bool == false){
				net_epd = get_pitd->net_epd;

				pthread_mutex_lock(&net_epd->nit->net_idx_mutex);
				net_idx = get_pitd->net_epd->nit->net_idx;
				if((net_idx + 1) == 0){
					net_ud.key = 1;
				}else{
					net_ud.key	= ( net_idx + 1 );
				}
				pthread_mutex_unlock(&net_epd->nit->net_idx_mutex);
			}
			pthread_mutex_unlock(&get_pitd->pitd_mutex);
...
#if 0
			if(data_send_idx_bool == false){
				pthread_mutex_lock(&net_epd->nit->net_idx_tree_mutex);
				net_pud = rb_find(net_epd->nit->net_idx_tree,(char *)&net_ud.key);
				pthread_mutex_unlock(&net_epd->nit->net_idx_tree_mutex);

				if(net_pud != NULL){
					get_pitd = net_pud->pitd;
					...
					if(get_pitd->NPT_send_ok == SEND_OK){
						if(get_pitd->packet_type == TUN_PACKET){
							if(get_pitd->NPT_packet_status == PACKET_DROP || get_pitd->TPT_packet_status == PACKET_DROP){
								...
								if(pipe_epd != NULL && pipe_epd->n_fdd != NULL){
									printf("##  %s %d %s pkt id = %lld key %lld T_TPT_idx %lld TPT_idx %lld ##\n",__func__,__LINE__,
													epd->name,
													get_pitd->data_send_idx,
													tmp_pitd.key,
													md->T_TPT_idx,
			...
			if(gitd_delete == true){
				...
				{
#if 0
					net_ud.key = get_pitd->data_send_idx;
					pthread_mutex_lock(&get_pitd->net_epd->nit->net_idx_tree_mutex);
					char *net_pud = rb_find(get_pitd->net_epd->nit->net_idx_tree,&net_ud);
					pthread_mutex_unlock(&get_pitd->net_epd->nit->net_idx_tree_mutex);
					if(net_pud != NULL){
						pthread_mutex_lock(&get_pitd->net_epd->nit->net_idx_tree_mutex);
						rb_delete(get_pitd->net_epd->nit->net_idx_tree,net_pud,true,0);
						pthread_mutex_unlock(&get_pitd->net_epd->nit->net_idx_tree_mutex);

						pthread_mutex_lock(&get_pitd->net_epd->nit->net_idx_mutex);
						get_pitd->net_epd->nit->net_idx = get_pitd->data_send_idx;
						pthread_mutex_unlock(&get_pitd->net_epd->nit->net_idx_mutex);
					}
					dz_pthread_mutex_destroy(&get_pitd->pitd_mutex);
#endif
...
//				if(ret_rb_delete != NULL)
      ...
      pthread_mutex_lock(&md->T_NPT_idx_mutex);
      if((md->T_NPT_idx) >= idx){
			//printf("############# %s %d %lld %lld ###\n",__func__,__LINE__,md->T_NPT_idx,idx);
			loop = false;
		}else{
			loop = true;
		}
					...
					if(epd->thd_mode == THREAD_PIPE_IN_OUT){
						if(opt->mode == SERVER){
							...
							if(ct_pud != NULL){
								...
								get_pitd->TPT_packet_status = PACKET_DROP;
								get_pitd->NPT_send_ok = SEND_OK;
								//printf("##  %s %d %s pkt id = %lld ##\n",__func__,__LINE__,epd->name,get_pitd->data_send_idx);
							}
						}else{
							pipe_epd = md->net_epd;
						}
...
//printf("###### %s %d len : %d pkt len : %d ##\n",__func__,__LINE__,data_ret,get_pitd->normal_packet_data_len-A_INTERNAL_HEADER-B_OPENVPN_PKT_SIZE-B_OPENVPN_KEYID_OPCODE);
									if(data_ret == 0){
						...
						if(gogo == true){
							if(get_pitd->NPT_packet_status == PACKET_DROP || get_pitd->TPT_packet_status == PACKET_DROP){
								...
								if(epd->thd_mode == THREAD_PIPE_IN_OUT){
									...
									if(pipe_epd != NULL && pipe_epd->n_fdd != NULL){
											//printf("##  %s %d %s pkt id = %lld key %lld T_TPT_idx %lld TPT_idx %lld ##\n",__func__,__LINE__,epd->name,get_pitd->data_send_idx,tmp_pitd.key,md->T_TPT_idx,md->TPT_idx);
											pthread_mutex_lock(&pipe_epd->n_fdd->net_w_mutex);
											ret = tcp_send(
													pipe_epd->n_fdd->net_wfd,
													(char *)get_pitd->encrypt_packet_data+A_INTERNAL_HEADER,
...
//printf("##  %s %d %s pkt id = %lld key %lld T_TPT_idx %lld TPT_idx %lld net_idx %lld data_send_idx %lld ##\n",__func__,__LINE__,epd->name,get_pitd->data_send_idx,tmp_pitd.key,md->T_TPT_idx,md->TPT_idx,get_pitd->net_epd->nit->net_idx,get_pitd->data_send_idx);
...
//printf("##  %s %d %s pkt id = %lld key %lld T_TPT_idx %lld TPT_idx %lld net_idx %lld data_send_idx %lld ##\n",__func__,__LINE__,epd->name,get_pitd->data_send_idx,tmp_pitd.key,md->T_TPT_idx,md->TPT_idx,get_pitd->net_epd->nit->net_idx,get_pitd->data_send_idx);
										...
										//printf("############# %s %d %lld  %lld %lld ############\n",__func__,__LINE__,get_pitd->key,get_pitd->net_epd->nit->net_idx,get_pitd->data_send_idx);
										//send_count++;
										//gitd_delete = false;
										data_send_idx_bool = false;
									}
...
//												printf("##  %s %d %s pkt id = %lld key %lld T_TPT_idx %lld TPT_idx %lld ##\n",__func__,__LINE__,epd->name,get_pitd->data_send_idx,tmp_pitd.key,md->T_TPT_idx,md->TPT_idx);
...
#if 0
			if(data_send_idx_bool == false && get_pitd != NULL && get_pitd->net_epd != NULL && get_pitd->net_epd->n_fdd != NULL){
				net_epd = get_pitd->net_epd;
				...
				if((net_idx + 1) == 0){
					...
					tmp_pitd.key	= ( net_idx + 1 );
				}
				pthread_mutex_unlock(&net_epd->nit->net_idx_mutex);

				pthread_mutex_lock(&net_epd->nit->net_idx_tree_mutex);
				get_pitd = rb_find(net_epd->nit->net_idx_tree,(char *)&tmp_pitd);
				pthread_mutex_unlock(&net_epd->nit->net_idx_tree_mutex);
			...
			if(gitd_delete == true){
				...
				if(ret_rb_delete != NULL)
				{
					...
					if(net_pud != NULL){
							pthread_mutex_lock(&get_pitd->net_epd->nit->net_idx_tree_mutex);
							rb_delete(get_pitd->net_epd->nit->net_idx_tree,net_pud,false,0);
							pthread_mutex_unlock(&get_pitd->net_epd->nit->net_idx_tree_mutex);
      ...
      pthread_mutex_lock(&md->T_TPT_idx_mutex);
      if((md->T_TPT_idx) >= idx){
			//printf("############# %s %d %lld %lld ###\n",__func__,__LINE__,md->T_TPT_idx,idx);
			loop = false;
		}else{
			loop = true;
		}
         ...
         pitd->recv_mil = ((tv.tv_sec*1000) + (tv.tv_usec/1000));
			...
			if(recv_len <= 0){
				...
				if(get_opcode((char *)pitd->encrypt_packet_data+A_INTERNAL_HEADER+A_INTERNAL_PKT_SIZE) != P_DATA_V1){
					...
					pthread_mutex_lock(&pitd->pitd_mutex);
					pitd->packet_type				= NETWORK_PACKET;
					pitd->key 						= packet_idx;
					pitd->NPT_send_ok 			= 0;
					pitd->NPT_packet_status 	= 0;
					pitd->TPT_send_ok 			= 0;
					pitd->TPT_packet_status 	= 0;
					pthread_mutex_unlock(&pitd->pitd_mutex);
					...
					pitd->NPT_packet_status 	= PACKET_DROP;
					pitd->TPT_send_ok 			= 0;//SEND_OK; 
					pitd->TPT_packet_status 	= 0;//PACKET_DROP;
					pthread_mutex_unlock(&pitd->pitd_mutex);
