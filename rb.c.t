...
struct rb_table *
rb_create (rb_comparison_func *compare, void *param,struct libavl_allocator *allocator,char *name)
{
...
rb_probe (struct rb_table *tree, void *item)
{
	...
	struct rb_node *p=NULL; /* Traverses tree looking for insertion point. */
	struct rb_node *n=NULL; /* Newly inserted node. */
	...
	if(pa[k - 1]->rb_link[da[k - 1]] != NULL){
		sfree(pa[k - 1]->rb_link[da[k - 1]],sizeof(struct rb_node));
	}
	//n = pa[k - 1]->rb_link[da[k - 1]] = tree->rb_alloc->libavl_malloc (tree->rb_alloc, sizeof *n);
	n = pa[k - 1]->rb_link[da[k - 1]] = malloc(sizeof(struct rb_node));
	if (n == NULL){
		printf("########################## %s %d ####\n",__func__,__LINE__);
		return NULL;
	}
...
void * rb_delete (struct rb_table *tree, const void *item, bool data_del,int data_size)
{
	struct rb_node *pa[RB_MAX_HEIGHT]; /* Nodes on stack. */
	unsigned char da[RB_MAX_HEIGHT];   /* Directions moved from stack nodes. */
	int k;                             /* Stack height. */

	struct rb_node *p = NULL;    /* The node to delete, or a node part way to it. */
	...
	if(p->rb_data != NULL && data_del == true){
		sfree(p->rb_data,data_size);
		//p->rb_data = NULL;
	}

	if(p != NULL){
		sfree(p,sizeof(struct rb_node));
		//p=NULL;
	}
		...
		rb_item_func *destroy, struct libavl_allocator *allocator)
{
	...
	struct rb_table *new = NULL;
	const struct rb_node *x = NULL;
	struct rb_node *y = NULL;

	assert (org != NULL);
	new = rb_create (org->rb_compare, org->rb_param,
			allocator != NULL ? allocator : org->rb_alloc,"rb_copy");
	if (new == NULL)
		return NULL;
...
{
	printf("################## %s %d ###############\n",__func__,__LINE__);
	void *p = rb_delete (table, item,true,0);
	assert (p != NULL);
	return p;
}
