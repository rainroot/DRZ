...
int verify_callback (void *epd_obj, x509_crt *cert, int cert_depth,int *flags)
{
...
char * backend_x509_get_serial (openvpn_x509_cert_t *cert)
{
	...
	if (mpi_read_binary(&serial_mpi, cert->serial.p, cert->serial.len) < 0)
	{
	...
	if (mpi_write_string(&serial_mpi, 10, buf, &buflen) < 0)
	{
